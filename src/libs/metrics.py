"""
Functions for dimensionality reduction evaluation
Supported measures:
- Local Measures
	- Trustworthiness & Continuity
	- Mean relative rank errors (MRRE)
- Cluster Measures
	- Steadiness & Cohesiveness
	- Class-Aware Trustworthiness & Continuity 
	- Label Trustworthiness & Continuity (DSC / CH_btwn)
- Global measures
  - KL Divergence
  - DTM
- CVM
...
from metrics_helper import GlobalMeasure, LocalMeasure
...
def class_aware_trust_conti(raw, emb, labels, k = 10):
	"""
	compute the Class Aware trustworthiness & continuity (ClassNeRV paper)
	"""
...
def ltnc_btw_ch(raw, emb, labels):
	"""
	Compute label-trustworthiness and label-continuity
	"""
	ltnc_obj = ltnc.LabelTNC(raw, emb, labels, cvm="btw_ch")
	return ltnc_obj.run()

def ltnc_dsc(raw, emb, labels):
	"""
	Compute distance consistency-based ltnc
	"""
	ltnc_obj = ltnc.LTNC(raw, emb, labels, cvm="dsc")
	return ltnc_obj.run()

def ltnc_btw_ch_time(raw, emb, labels):
	"""
...
def ltnc_dsc_time(raw, emb, labels):
	"""
...
def class_aware_trust_conti_time(raw, emb, labels, k = 10):
	"""
	compute the class_aware trustworthiness and continuity
	"""
	start = time.time()
	class_aware_trust_conti(raw, emb, labels, k = k)
	end = time.time()
	return end - start
